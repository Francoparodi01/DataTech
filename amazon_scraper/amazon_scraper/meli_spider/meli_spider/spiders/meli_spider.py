...
class Scraper():
    ...
    def scraping(self):
        ...
        for i, url in enumerate(urls, start=1):
            ...
            for post in content:
                # Depuración: Imprimir el contenido de post
                # print(post)

                try:
                    title = post.find('h2', class_='ui-search-item__title').text.strip()
                except AttributeError:
                    title = "Título no disponible"

                try:
                    price = post.find('span', class_='andes-money-amount__fraction').text.replace('.', '').replace(',', '.')
                except AttributeError:
